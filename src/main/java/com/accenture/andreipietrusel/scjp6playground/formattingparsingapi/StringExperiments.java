...
public class StringExperiments {
	...
	public static void main(String[] args) {
		...
		s.intern(); // already in pool, doesnt replace it the pool one, returns old pooled object
		System.out.println(s == sp); // this will be false
		
		String rev = s2.concat(s1);
		rev.intern(); // adds exactly this string to pool, returns same (not used here)
